...
import { globalStoragePath, taskBaseDir } from "../../const.js"
...
class CheckpointTracker {
	...
	private constructor(taskId: string, cwd: string) {
		this.taskId = taskId
		this.cwd = cwd
	}

	public static async create(taskId: string): Promise<CheckpointTracker | undefined> {
		try {
			...
			const newTracker = new CheckpointTracker(taskId, cwd)
	...
	private async getShadowGitPath(): Promise<string> {
		const checkpointsDir = path.join(taskBaseDir, this.taskId, "checkpoints")
		await fs.mkdir(checkpointsDir, { recursive: true })
		const gitPath = path.join(checkpointsDir, ".git")
		return gitPath
	}

	public static async doesShadowGitExist(taskId: string): Promise<boolean> {
